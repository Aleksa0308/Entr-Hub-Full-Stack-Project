...
export default {
    ...
    mounted() {
        ...
        this.form.issue = this.singleItem.issue
        this.form.publisher = this.singleItem.publisher
    },
    methods: {
        ...mapActions([
            'fetchComic',
        ...
        onSubmit(e) {
            e.preventDefault();
            this.id = this.$route.params.id;
            this.updateComics({obj: this.form, id: this.id});
            this.$router.push({ name: 'comicBooks' })
                .then(() => { this.$router.go() });
        },
        deleteOne(){
            var result = window.confirm("Are you sure you want to delete this comicbook?")
            if (result) {
                this.id = this.$route.params.id;
                this.deleteComics(this.id);
                this.$router.push({ name: 'comicBooks' })
                    .then(() => { this.$router.go() });
            }
        }
