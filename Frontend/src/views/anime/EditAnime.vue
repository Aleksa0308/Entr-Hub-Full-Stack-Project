...
export default {
    ...
    mounted() {
        ...
        this.form.manga = this.singleItem.manga
        this.form.epizodes = this.singleItem.epizodes
        this.id = this.$route.params.id;
        this.fetchAni(this.id);
    ...
    methods: {
        ...
        onSubmit(e) {
            e.preventDefault();
            this.id = this.$route.params.id;
            this.updateAnime({obj: this.form, id: this.id});
            this.$router.push({ name: 'anime' })
                .then(() => { this.$router.go() });
        },
        deleteOne(){
            var result = window.confirm("Are you sure you want to delete this anime?")
            if (result) {
                this.id = this.$route.params.id;
